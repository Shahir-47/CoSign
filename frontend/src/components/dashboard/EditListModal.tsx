...
export default function EditListModal({
	...
	// Initialize form when list changes or modal opens
	useEffect(() => {
		if (isOpen && list) {
			setName(list.name);
			setSelectedIcon(list.icon || "folder");
			setSelectedColor(list.colorHex || COLORS[0]);
			setError(undefined);
		}
	}, [isOpen, list]);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const updatedList = await api.put<TaskList>(`/lists/${list.id}`, request);
			toast.success(`List "${updatedList.name}" updated!`);
			...
			<div className={styles.modal} onClick={(e) => e.stopPropagation()}>
				...
				<form onSubmit={handleSubmit} className={styles.form}>
					<div className={styles.preview}>
						...
						<span className={styles.previewName}>{name || "List Name"}</span>
					</div>
